...
public class LinkifyUtils {
    ...
    public static boolean linkifys(TextView textView, StringBuilder text,
        ...
        //add 2
         ...
         int beginIndex2 = text.indexOf(PLACE_HOLDER_LINK_BEGIN);
        if (beginIndex2 == -1) {
            textView.setText(text);
            return false;
        }
        text.delete(beginIndex2, beginIndex2 + PLACE_HOLDER_LINK_BEGIN.length());
         int endIndex2 = text.indexOf(PLACE_HOLDER_LINK_END);
        if (endIndex2 == -1) {
            textView.setText(text);
            return false;
        }
        text.delete(endIndex, endIndex + PLACE_HOLDER_LINK_END.length());
        textView.setText(text.toString(), BufferType.SPANNABLE);
        Spannable spannableContent2 = (Spannable) textView.getText();
        ClickableSpan spannableLink2 = new ClickableSpan() {
            @Override
            public void onClick(View widget) {
                listener2.onClick();
                widget.invalidate();
        ...
        spannableContent2.setSpan(spannableLink, beginIndex, endIndex,
                Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        spannableContent2.setSpan(spannableLink2, beginIndex2, endIndex2,
                Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
